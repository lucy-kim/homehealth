...
loc vars epiid provider_hosp socdate* offid_nu clientid fy inpat_dcd hashosp* days2hosp age ami hf pneu copd
keep `chars' `vars'
duplicates drop

rename pneu pn
foreach d in "ami" "hf" "pn" "copd" {
  rename `d' `d'_byalldx
}

*merge with data on first inpatient DX per patient
merge 1:1 clientid socdate_e using inpat_firstdx, keep(3) nogen
...
tab hrrpcond_count
assert hrrpcond_count==1 | hrrpcond_count==0
...
compress
save epilvl_rehosp_smpl_old_firstdx, replace
