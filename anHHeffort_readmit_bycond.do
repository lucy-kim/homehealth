...
foreach d in "ami" "hf" "pn" {
  use `smpl_tm1', clear
  assert pnltprs_c==0 if hrrpcond==0
  replace pnltprs_c = pnltprs_`d'  if hrrpcond==0
  loc pp `d' pnltprs_c pnltprs_c_X_`d'
  keep if `d'==1 | hrrpcond==0
  gen nontarget = 1-hrrpcond
  assert `d'+nontarget==1
...
foreach d in "ami" "hf" "pn" {
  ...
  loc iv pnltprs_c pnltprs_c_X_`d'
  ...
  *initiate control list
  ...
  forval nn = 1/3 {
    loc ev1 `end`nn''
    eststo y_n`nn': ivreg2 `yv' `sp' `d'  i.offid_nu (`end`nn'' = `iv'), cluster(offid_nu) first savefirst savefprefix(f_n`nn'_) gmm2s partial(i.fy i.offid_nu ) endog(`end`nn'')
    ...
    *get R-squared from first stage OLS regression
    areg `end`nn'' `iv' `sp' `d', absorb(offid_nu) vce(cluster offid_nu)
